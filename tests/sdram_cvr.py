...
@CoverPoint("top.address",
			xf=lambda x, y, z: y,
			bins = [0x000, 0xFFF]
			)
@CoverPoint("top.transaction",
			xf=lambda x, y, z: z,
			bins = ['read', 'write']
			)			
def data_adr_tx_cover(data, address, transaction):
	pass
...
async def sdram_controller_test(dut):
    ...
    # Monitor
    monitor = BusMonitor(dut, "", dut.clk0)
    monitor_thread = cocotb.fork(monitor.start())
    if dut.we == 0:
        ...
        for addr, data in zip(random_data_addresses, random_data_values):
            ...
            await RisingEdge(dut.clk0)
            scoreboard.append(await scoreboard.read(dut.sdram_data, data, "Read data mismatch"))
            data_adr_tx_cover(data, address, transaction)
        coverage_db.report_coverage(cocotb.lg.info, bins=True)
        coverage_file = os.path.join(
        	os.getenv('RESULT_PATH', "./"), 'coverage.xml')
        coverage_db.export_to_xml(filename=coverage_file)
        
    coverage_db.finalize()
    coverage_db.report()
    monitor_thread.kill()
